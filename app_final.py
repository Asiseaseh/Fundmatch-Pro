...
.chip-F01 {{ background:#e8f5e9; color:#1b5e20; }}
.chip-F02 {{ background:#e3f2fd; color:#0d47a1; }}
.chip-F03 {{ background:#e8eaf6; color:#283593; }}
.chip-F04 {{ background:#fff9c4; color:#e65100; }}
.chip-F05 {{ background:#fff3e0; color:#bf360c; }}
.chip-F06 {{ background:#fce4ec; color:#880e4f; }}
.chip-F07 {{ background:#ede7f6; color:#4527a0; }}
.chip-F08 {{ background:#ffebee; color:#b71c1c; }}
.chip-F09 {{ background:#f3e5f5; color:#6a1b9a; }}
.chip-F10 {{ background:#dcfce7; color:#14532d; }}
.chip-F11 {{ background:#eceff1; color:#263238; }}
/* PROFILE BADGE */
.profile-badge {{
...
# ─────────────────────────────────────────────────────────────────────────────
FAMILIA_CONFIG = {
    "F01 – Monetario Puro":              {"label":"F01 — Monetario Puro",          "color":"#1b5e20","bg":"#e8f5e9","chip":"chip-F01","risk":"Muy Bajo",      "vol":"< 1%",    "desc":"Fondos del mercado monetario. Preservación total del capital. Rentabilidad ligada al Euribor overnight."},
    "F02 – RF Corto Plazo IG":           {"label":"F02 — RF Corto Plazo IG",       "color":"#0d47a1","bg":"#e3f2fd","chip":"chip-F02","risk":"Muy Bajo-Bajo", "vol":"0,5%–4%", "desc":"Renta fija de alta calidad crediticia y duración corta. TER reducido como criterio clave."},
    "F03 – RF IG Diversificada":         {"label":"F03 — RF IG Diversificada",     "color":"#283593","bg":"#e8eaf6","chip":"chip-F03","risk":"Bajo",           "vol":"2%–8%",   "desc":"RF de grado de inversión diversificada. Gubernamental y corporativa IG."},
    "F04 – RF Flexible / HY / EM":       {"label":"F04 — RF Flexible/HY/EM",       "color":"#e65100","bg":"#fff9c4","chip":"chip-F04","risk":"Bajo-Medio",     "vol":"4%–12%",  "desc":"High yield, emergentes y RF flexible. Mayor rentabilidad potencial con más volatilidad."},
    "F05 – Mixto Conservador":           {"label":"F05 — Mixto Conservador",       "color":"#bf360c","bg":"#fff3e0","chip":"chip-F05","risk":"Medio",           "vol":"3%–8%",   "desc":"Carteras mixtas defensivas (60-80% RF). Primer escalón con exposición a renta variable."},
    "F06 – Mixto Equilibrado":           {"label":"F06 — Mixto Equilibrado",       "color":"#880e4f","bg":"#fce4ec","chip":"chip-F06","risk":"Medio",           "vol":"6%–12%",  "desc":"Carteras mixtas balanceadas (40-60% RV). El referente de la banca privada española."},
    "F07 – Mixto Dinámico / Asset Allocation":              {"label":"F07 — Mixto Dinámico",          "color":"#4527a0","bg":"#ede7f6","chip":"chip-F07","risk":"Medio-Alto",      "vol":"10%–18%", "desc":"Carteras mixtas agresivas (60-80% RV). Gestión activa de la asignación de activos."},
    "F08 – RV Mercados Desarrollados":   {"label":"F08 — RV Mercados Desarrollados","color":"#b71c1c","bg":"#ffebee","chip":"chip-F08","risk":"Alto",           "vol":"10%–20%", "desc":"RV diversificada de EEUU, Europa y Japón. Base sólida para carteras de crecimiento."},
    "F09 – RV Crecimiento / Sectorial / Temática":  {"label":"F09 — RV Crecimiento/Sectorial","color":"#6a1b9a","bg":"#f3e5f5","chip":"chip-F09","risk":"Muy Alto",       "vol":"14%–35%", "desc":"RV de alta convicción: tecnología, salud, emergentes, small caps. Máximo potencial."},
    "F10 – Activos Reales e Inflación":  {"label":"F10 — Activos Reales/Inflación","color":"#14532d","bg":"#dcfce7","chip":"chip-F10","risk":"Medio-Alto",      "vol":"8%–18%",  "desc":"REITs, materias primas y RF ligada a inflación. Cobertura frente al IPC."},
    "F11 – Alternativo y Retorno Absoluto":{"label":"F11 — Alternativo/Ret.Absoluto","color":"#263238","bg":"#eceff1","chip":"chip-F11","risk":"Variable",      "vol":"Variable","desc":"Retorno absoluto, market neutral, arbitraje. Baja correlación con mercados tradicionales."},
}

PERFILES_META = {
    1:  {"nombre":"Conservador de Liquidez",   "fam":"F01 – Monetario Puro",             "riesgo":"Muy Bajo",    "horizonte":"< 6 meses"},
    2:  {"nombre":"Conservador de Ingresos",   "fam":"F02 – RF Corto Plazo IG",          "riesgo":"Bajo",        "horizonte":"6-24 meses"},
    3:  {"nombre":"Moderado Renta Fija",       "fam":"F03 – RF IG Diversificada",        "riesgo":"Bajo-Medio",  "horizonte":"1-5 años"},
    4:  {"nombre":"Mixto Conservador",         "fam":"F05 – Mixto Conservador",          "riesgo":"Medio",       "horizonte":"2-5 años"},
    5:  {"nombre":"Mixto Equilibrado",         "fam":"F06 – Mixto Equilibrado",          "riesgo":"Medio",       "horizonte":"3-7 años"},
    6:  {"nombre":"Protección ante Inflación", "fam":"F10 – Activos Reales e Inflación", "riesgo":"Medio-Alto",  "horizonte":"3-10 años"},
    7:  {"nombre":"Mixto Dinámico",            "fam":"F07 – Mixto Dinámico / Asset Allocation",             "riesgo":"Medio-Alto",  "horizonte":"5-10 años"},
    8:  {"nombre":"RV Mercados Desarrollados", "fam":"F08 – RV Mercados Desarrollados",  "riesgo":"Alto",        "horizonte":"7-15 años"},
    9:  {"nombre":"RV Crecimiento / Sectorial","fam":"F09 – RV Crecimiento / Sectorial / Temática", "riesgo":"Muy Alto",    "horizonte":"10+ años"},
    10: {"nombre":"Activos Reales y Alternativo","fam":"F11 – Alternativo y Retorno Absoluto","riesgo":"Variable","horizonte":"3-10 años"},
}
...
def cargar_datos():
    """
    Carga el Excel generado por clasificador_fondos.py y normaliza las
    columnas al formato interno que usa el resto de la app.

    PRIORIDAD:
      1. Fondos_Clasificados_COMPLETO.xlsx  ← generado por clasificador_fondos.py
      2. Base_de_datos_fondos.xlsx          ← fuente original (fallback sin familias)
    """
    COMPLETO = "Fondos_Clasificados_COMPLETO.xlsx"
    # Versión de la función - cambiar este número fuerza el refresco de caché
    _VERSION = "2.1"  # noqa

    if not os.path.exists(COMPLETO):
        st.error(
            "❌ No se encontró **Fondos_Clasificados_COMPLETO.xlsx**.\n\n"
            "Ejecuta primero: `python clasificador_fondos.py`"
        )
        st.stop()

    df = pd.read_excel(COMPLETO, sheet_name="Fondos_Clasificados")

    # ── Mapa de columnas: nombre en Excel → nombre interno de la app ───────
    # El clasificador genera columnas con nombres descriptivos completos.
    # La app usa nombres internos cortos. Este mapa hace la traducción.
    # Mapa COMPLETO y exacto de nombres del Excel → nombres internos de la app
    # Verificado contra la salida real del clasificador_fondos.py
    col_map = {
        # Identificación
        "Nombre del Fondo":            "Name",
        "Categoría Morningstar":       "Morningstar Category",
        "Divisa":                      "Currency",
        # Familia — NOTA: el Excel escribe "FAMILIA (1-11)", no "FAMILIA"
        "FAMILIA (1-11)":              "Familia_Codigo",
        "Nombre Familia":              "Familia_Perfil",
        "Tipo de Activo":              "Tipo_Activo",
        "Geografía":                   "Geográfia",
        "Zona Divisa":                 "Zona_Divisa",
        "Criterios Asignación":        "Justificacion",
        # Riesgo
        "SRRI Regulatorio":            "SRRI",
        "SRRI Imputado":               "SRRI_Imputado",
        "Vol. 3A (%)":                 "Vol_3Y",
        "Vol. Media 6A (%)":           "Vol_Med_6A",
        "MaxDD Peor (%)":              "MaxDrawdown",
        "MaxDD Medio 6A (%)":          "DD_Med_6A",
        # Rentabilidad — espacios exactos del Excel
        "Rent. 1A (%)":                "Ret_1Y",
        "Rent. 3A Anualiz. (%)":       "Ret_3Y",    # con espacio antes de (%)
        "Rent. 5A Anualiz. (%)":       "Ret_5Y",    # con espacio antes de (%)
        "Rent. Media 6A (%)":          "Ret_Med_6A",
        "Rent. 2022 (%)":              "Ret_2022",
        "Rent. 2023 (%)":              "Ret_2023",
        "Rent. 2024 (%)":              "Ret_2024",
        # Calidad
        "Rating (1-5★)":               "Morningstar Rating",
        "Sharpe 3A":                   "Sharpe_3Y",
        "Consistencia (%)":            "Consistencia",
        "% Años Positivos":            "Pct_Anos_Pos",
        "Recup. Post-2022 (%)":        "Recuperacion_2022",
        "P/E Ratio":                   "PE_Ratio",
        # Coste y tamaño
        "TER / Gastos (%)":            "TER_KIID",
        "AUM (€)":                     "AUM_EUR",
        "Inversión Mínima":            "MinInv",    # sin "(€)" en el Excel real
        # Score global
        "◆ Score Calidad (0-100)":     "Score_Calidad",
        # Scores por perfil — nombre exacto del Excel
        "◆ Score P01":                 "Score_P01",
        "◆ Score P02":                 "Score_P02",
        "◆ Score P03":                 "Score_P03",
        "◆ Score P04":                 "Score_P04",
    ...
    # ── Columnas derivadas para compatibilidad con el resto de la app ──────
    # Familia_L2
    if "Familia_L2" not in df.columns:
        df["Familia_L2"] = df.get("Tipo_Activo", "Otro")

    # Riesgo_Banda: derivada del SRRI Imputado
    if "Riesgo_Banda" not in df.columns:
        def srri_a_banda(s):
            try:
                s = float(s)
                if s <= 2: return "Riesgo Muy Bajo"
                if s <= 3: return "Riesgo Bajo"
                if s <= 4: return "Riesgo Medio"
                if s <= 5: return "Riesgo Medio-Alto"
                if s <= 6: return "Riesgo Alto"
                return "Riesgo Muy Alto"
            except:
                return "Riesgo n/d"
        df["Riesgo_Banda"] = df["SRRI_Imputado"].apply(srri_a_banda)

    # ESG_Flag: garantizar que siempre existe aunque no viniera en el Excel
    if "ESG_Flag" not in df.columns:
        ESG_KW = ["esg","sustain","responsible","green","impact","sri",
                  "climate","ethical","environ","low carbon","transition",
                  "clean","renewable","conscious","nordic"]
        def detectar_esg(row):
            texto = (str(row.get("Name","")) + " " +
                     str(row.get("Morningstar Category",""))).lower()
            return 1 if any(k in texto for k in ESG_KW) else 0
        df["ESG_Flag"] = df.apply(detectar_esg, axis=1)
    else:
        df["ESG_Flag"] = pd.to_numeric(df["ESG_Flag"], errors="coerce").fillna(0).astype(int)

    # ── Tipos numéricos ───────────────────────────────────────────────────
    num_cols = ["AUM_EUR","Vol_3Y","MaxDrawdown","Ret_1Y","Ret_3Y","TER_KIID",
                "Pct_Anos_Pos","Ret_2022","Ret_2023","Ret_2024","DD_2022",
                "SRRI_Imputado","Score_Calidad","ESG_Flag"]
    for col in num_cols:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce")

    # AUM en millones (para mostrar en la app)
    df["AUM_M"] = df["AUM_EUR"].fillna(0) / 1e6

    # Score columna única para el recomendador (se sobreescribe por perfil)
    if "Score" not in df.columns:
        df["Score"] = df.get("Score_Calidad", 0)

    return df

@st.cache_data(show_spinner=False)
def stats_familias(df):
    grp = df.groupby("Familia_Perfil").agg(
        N=("Name","count"),
        Vol_med=("Vol_3Y","median"),
        Ret_med=("Ret_1Y","median"),
        DD_med=("MaxDrawdown","median"),
        TER_med=("TER_KIID","median"),
        Sharpe_med=("Sharpe_3Y","median"),
    ).round(2).reset_index()
    return grp
...
# Aviso si el Excel no tiene las columnas de score por perfil (Excel desactualizado)
_tiene_scores = "Score_P01" in fondos.columns or "Score_P09" in fondos.columns
if not _tiene_scores:
    st.warning(
        "⚠️ **El Excel actual no tiene las columnas de Score por perfil.** "
        "Para obtener recomendaciones, ejecuta primero:\n\n"
        "```bash\npython clasificador_fondos.py\n```\n\n"
        "Esto regenera `Fondos_Clasificados_COMPLETO.xlsx` con todas las columnas necesarias."
    )
...
# Familias correctas por perfil (doble garantía además del Excel)
PERFIL_FAMILIAS = {
    1:  ["F01","F02"],          # Conservador de Liquidez
    2:  ["F02","F03"],          # Conservador de Ingresos
    3:  ["F03","F04"],          # Moderado Renta Fija
    4:  ["F05"],                # Mixto Conservador
    5:  ["F06"],                # Mixto Equilibrado
    6:  ["F04","F10"],          # Protección ante Inflación
    7:  ["F07"],                # Mixto Dinámico
    8:  ["F08"],                # RV Mercados Desarrollados
    9:  ["F09"],                # RV Crecimiento / Sectorial
    10: ["F10","F11"],          # Activos Reales y Alternativo
}

# Mapa de restricciones de usuario → Tipos_Activo a excluir
RESTRICCIONES_MAP = {
    "Sin Renta Variable":      ["RV"],
    "Sin Renta Fija":          ["RF_IG","RF_HY","RF_EM","RF_Inflation","Monetario"],
    "Sin Inmobiliario":        ["Inmobiliario"],
    "Sin Materias Primas":     ["MatPrimas"],
    "Sin Alternativos":        ["Alternativo"],
    "Sin High Yield":          ["RF_HY"],
    "Sin Emergentes":          ["RF_EM"],
    "Sin Deuda Subordinada":   ["RF_HY"],
}

def recomendar(df, pnum, restricciones, capital, top_n=15):
    score_col = f"Score_P{pnum:02d}"

    # Si no existe la columna de score, el Excel es antiguo → fallback a Score_Calidad
    if score_col not in df.columns:
        # Intentar con Score_Calidad como fallback
        if "Score_Calidad" not in df.columns:
            return pd.DataFrame()
        score_col = "Score_Calidad"

    # Filtro 1: familias elegibles para este perfil
    fams = PERFIL_FAMILIAS.get(pnum, [])

    if "Familia_Codigo" in df.columns:
        # Caso normal: columna Familia_Codigo con códigos F01-F11
        mask = df["Familia_Codigo"].isin(fams)
    elif "Familia_Perfil" in df.columns:
        # Fallback: Familia_Perfil tiene nombres largos tipo "F09 – RV Crecimiento..."
        # Extraemos el código F01-F11 del principio del string
        df["_fam_code"] = df["Familia_Perfil"].str.extract(r"^(F\d{2})", expand=False)
        mask = df["_fam_code"].isin(fams)
    else:
        return pd.DataFrame()

    # Filtro 2: capital mínimo del inversor
    if capital > 0:
        if "MinInv" in df.columns:
            mask &= df["MinInv"].fillna(0) <= capital

    # Filtro 3: restricciones de tipo de activo seleccionadas por el usuario
    for r in restricciones:
        excl = RESTRICCIONES_MAP.get(r, [])
        if excl:
            mask &= ~df["Tipo_Activo"].isin(excl)

    sub = df[mask].copy()
    if sub.empty:
        return sub

    # Ordenar por el score específico del perfil
...
if st.sidebar.button("🔄 Actualizar datos", help="Recarga el Excel y borra la caché"):
    st.cache_data.clear()
    st.rerun()
...
# FAM_COLORS: built from FAMILIA_CONFIG + fallback palette for any family not in config
FAM_COLORS = {k: FAMILIA_CONFIG[k]["color"] for k in FAMILIA_CONFIG}
# Add fallback colors for any family that exists in data but not in FAMILIA_CONFIG
_FALLBACK_PALETTE = ["#1b5e20","#0d47a1","#283593","#e65100","#bf360c",
                     "#880e4f","#4527a0","#b71c1c","#6a1b9a","#14532d","#263238"]
if hasattr(fondos, "columns") and "Familia_Perfil" in fondos.columns:
    for _i, _fam in enumerate(fondos["Familia_Perfil"].dropna().unique()):
        if _fam not in FAM_COLORS:
            FAM_COLORS[_fam] = _FALLBACK_PALETTE[_i % len(_FALLBACK_PALETTE)]

# ─────────────────────────────────────────────────────────────────────────────
# PÁGINA 1 — RESUMEN DEL UNIVERSO
# ─────────────────────────────────────────────────────────────────────────────
if pagina == "Resumen del Universo":
    n_fam = fondos["Familia_Perfil"].nunique()
    page_hdr("Universo de Fondos", f"Base Morningstar EAA — {len(fondos):,} fondos clasificados en {n_fam} familias de inversión")

    cols = st.columns(5)
    n_fam_real = fondos["Familia_Perfil"].nunique()
    kpis = [
        ("Fondos totales",  f"{len(fondos):,}",              "Base Morningstar EAA"),
        ("Familias activas", str(n_fam_real),                "universo clasificado"),
        ("Tipos de activo",  str(fondos["Tipo_Activo"].nunique() if "Tipo_Activo" in fondos.columns else "—"), "en el universo"),
    ...
    with c1:
        ...
        fam_activa = st.session_state.get("fam_filtro", None)
        dist_plot = dist.copy()
        dist_plot["opacity"] = dist_plot["Familia"].apply(
            lambda f: 1.0 if (fam_activa is None or f == fam_activa) else 0.25)
        fig = px.bar(dist_plot, x="Fondos", y="Familia", orientation="h",
                     color="Familia", color_discrete_map=FAM_COLORS, text="Fondos",
                     opacity=1.0)
        for i, row_d in dist_plot.iterrows():
            fig.data[i if i < len(fig.data) else 0].opacity = row_d["opacity"]
        fig.update_layout(showlegend=False, height=300, font=dict(family="Inter",size=11),
        ...
        fig.update_traces(textposition="outside", marker_line_width=0, selectedpoints=[])
        clicked = st.plotly_chart(fig, use_container_width=True,
                                  config={"displayModeBar": False}, on_select="rerun",
                                  selection_mode="points", key="bar_familias")
        # Procesar click
        if clicked and hasattr(clicked, "selection") and clicked.selection.get("points"):
            fam_clicked = clicked.selection["points"][0].get("label") or clicked.selection["points"][0].get("y")
            if fam_clicked:
                if st.session_state.get("fam_filtro") == fam_clicked:
                    st.session_state["fam_filtro"] = None  # segundo click = deseleccionar
                else:
                    st.session_state["fam_filtro"] = fam_clicked
                st.rerun()
        if fam_activa:
            st.caption(f"🔍 Filtrando por: **{fam_activa}** — [Quitar filtro]({'#'})")
            if st.button("✕ Quitar filtro de familia", key="quitar_fam"):
                st.session_state["fam_filtro"] = None
                st.rerun()
    ...
    # Aplicar filtro de familia si está activo (click en barra)
    fam_activa_tabla = st.session_state.get("fam_filtro", None)
    stats_fam_show = stats_fam[stats_fam["Familia_Perfil"] == fam_activa_tabla] if fam_activa_tabla else stats_fam
    for _, row in stats_fam_show.iterrows():
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    # ── RESULTADOS ────────────────────────────────────────────────────────────
    if buscar:
        sc_col = f"Score_P{perfil_num:02d}"
        ...
        if resultado.empty:
            ...
            with tabs[0]:
                ...
                for _, r in resultado.iterrows():
                    ...
                    with cc[1]:
                        ...
                        <span style='font-size:.7rem;color:{'#8b949e' if dark else '#8a9ab0'}'>{r.get('Tipo_Activo','')}</span>""", unsafe_allow_html=True)
            ...
            with tabs[1]:
                ...
                if not plot_df.empty:
                    fig_sc = px.scatter(plot_df, x="Vol_3Y", y="Ret_1Y",
                                        ...
                                        hover_name="Name",
                                        hover_data={"TER_KIID":":.2f","Score":":.1f","Tipo_Activo":True},
            ...
            with tabs[2]:
                ...
                with col_sub:
                    ...
                    sub_conc = resultado.groupby("Tipo_Activo")["Name"].count().reset_index()
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    # Sólo incluir columnas que existan; los nombres van pareados
    present = [(col, lbl) for col, lbl in show_cols if col in sub.columns]
    display = sub[[col for col, _ in present]].head(200).copy()
    display.columns = [lbl for _, lbl in present]
    ...
    fmt_dict = {
        ...
        "Sharpe":   lambda v: f"{v:.2f}" if not pd.isna(v) else "—",
        "ESG":      lambda v: "✓" if v == 1 else "",
    }
    # Solo aplicar formatos a columnas que existen en display
    active_fmt = {k: v for k, v in fmt_dict.items() if k in display.columns}

    st.dataframe(
        display.style.format(active_fmt),
    ...
    if nombres:
        ...
        if not det.empty:
            ...
            st.markdown(f"""<br><div style="font-size:.8rem;color:{'#8b949e' if dark else '#6a8aaa'};line-height:1.8">
              ...
              <b>Tipo de activo:</b> {det.get('Tipo_Activo','—')}<br>
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    page_hdr("Familias de Inversión", "Análisis de las 11 familias y sus características")
    ...
    with tab_sf:
        sub_dist = sub_fam.groupby("Tipo_Activo").agg(
            N=("Name","count"),
            Vol_med=("Vol_3Y","median"),
            Ret_med=("Ret_1Y","median"),
            TER_med=("TER_KIID","median"),
        ...
        st.dataframe(sub_dist.rename(columns={"Tipo_Activo":"Sub-familia","N":"Fondos",
    ...
    with tab_top:
        sc_ref = "Score_Calidad" if "Score_Calidad" in sub_fam.columns else "Score"
        if sc_ref:
            top15 = sub_fam.nlargest(15, sc_ref)[["Name","ISIN","Tipo_Activo","Riesgo_Banda","Morningstar Rating","Ret_1Y","Vol_3Y","Sharpe_3Y","TER_KIID","AUM_M"]].reset_index(drop=True)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    if seleccion:
        ...
        metricas_comp = {
            ...
            "Tipo de activo":    ("Tipo_Activo",          None),
